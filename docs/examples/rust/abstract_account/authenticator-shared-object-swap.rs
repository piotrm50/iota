...
//! Example demonstrating the shared object swap vulnerability in an abstract
//! account authenticator implementation.
...
const AA_PACKAGE_PATH: &str = "../move/abstract_account/authenticator-shared-object-swap";
...
/// Add to blacklist function name.
const AA_ADD_TO_BLACKLIST_FN_NAME: &str = "add";
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    // Create a blacklist shared object instance
    let blacklist_ref =
...
pub async fn add_address_to_blacklist(
    ...
    address_to_add: IotaAddress,
) -> Result<()> {
    // Create a PTB that adds an address to the blacklist
    let pt = {
        let mut builder = ProgrammableTransactionBuilder::new();
    ...
    println!(
        "Blacklist adding transaction digest: {}",
        transaction_response.digest
    );
