...
impl TestCheckpointDataBuilder {
    ...
    /// `object_idx` is a convenient representation of the object's ID.
    /// `owner_idx` is a convenient representation of the object's owner's
    /// address. `balance` is the amount of IOTA to be created.
    ...
    /// `object_idx` is a convenient representation of the object's ID.
    /// `recipient_idx` is a convenient representation of the recipient's
    /// address.
    ...
    /// `recipient_idx` is a convenient representation of the recipient's
    /// address. `amount` is the amount of balance to be transferred.
    ...
    /// This will also finalize all the object changes, and reflect them in the
    /// live object map.
    ...
    /// builder so far. This will also increment the stored checkpoint
    /// sequence number.
...
mod tests {
    ...
    fn test_multiple_transactions() {
        ...
        // Verify transactions have different senders (since we used 0, 1, 2 as sender
        // indices above).
    ...
    fn test_iota_balance_transfer() {
        ...
        // Verify the newly created object appears in output objects and is a gas coin
        // with 100 NANOS.
