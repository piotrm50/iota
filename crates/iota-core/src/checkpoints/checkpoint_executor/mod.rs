...
impl CheckpointExecutor {
    ...
    /// Execute all checkpoints for the current epoch, ensuring that the node
    /// has not forked, and return when finished.
    ...
    pub async fn run_epoch(self, run_with_range: Option<RunWithRange>) -> StopReason {
        let _metrics_scope = iota_metrics::monitored_scope("CheckpointExecutor::run_epoch");
...
impl CheckpointExecutor {
    ...
    async fn execute_change_epoch_tx(&self, tx_data: &CheckpointTransactionData) {
        ...
        // Ordinarily we would assert that the change epoch txn has not been executed
        // yet. However, during crash recovery, it is possible that we already
        // passed this point and the txn has been executed. You can uncomment
        // this assert if you are debugging a problem related to reconfig. If
        // you hit this assert and it is not because of crash-recovery,
    ...
    fn extract_randomness_rounds(
            ...
            .expect("unable to get version_specific_data")
        {
            ...
            // because all other transactions in a checkpoint that includes a
            // randomness state update are causally dependent on it.
            assert_eq!(
                0,
                self.epoch_store
                    .protocol_config()
