...
use iota_types::{
    ...
    messages_consensus::{
        AuthorityCapabilitiesV1, ConsensusDeterminedVersionAssignments, ConsensusTransaction,
...
async fn test_post_consensus_white_flag_simple_conflict() {
    ...
    let lock = get_object_lock(&epoch_store, &object.compute_object_reference());
    assert_eq!(
        lock,
...
async fn test_post_consensus_white_flag_no_conflict() {
    ...
    let lock1 = get_object_lock(&epoch_store, &object1.compute_object_reference());
    assert_eq!(
        lock1,
        Some(*verified_tx1.digest()),
    ...
    let lock2 = get_object_lock(&epoch_store, &object2.compute_object_reference());
    assert_eq!(
        lock2,
